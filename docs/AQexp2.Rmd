...
### note to self - throw in a boxplot of light vs day/night and scatter of light vs temp
```
...
airDF <- airDF %>%
  mutate(Date = lubridate::dmy(Date), Time=lubridate::hms(Time))
  # We want to code which observations are made in the day vs. night
  # using 5pm as our cut-off
pm_limit <- lubridate::hms("17:00:00")
...
```{r}
resample(airDF[,c("Date","Time","Lux","DayNight")],groups=DayNight,shuffled=c("Lux"))
```
